...
[ ] 任务9：网络连接诊断工具（详细设计）
  - 目标：
    - [ ] 实现智能网络连接诊断工具，快速识别Wikipedia连接问题的根本原因。
    - [ ] 提供分层诊断能力，区分网络问题和程序设计问题。
    - [ ] 集成到MCP服务器中，支持自动化和手动诊断。
  - 技术架构设计：
    - **分层诊断策略**：
    ...
    - **智能分析引擎**：
      ...
      - 历史问题模式识别（可选）
  - 步骤：
    - [ ] 在 src/constants.ts 中添加新工具常量：
      - `NETWORK_DIAGNOSTIC` - 网络诊断工具名称
    - [ ] 在 src/index.ts 中实现诊断处理器：
  ...
  - 接口设计规范：
    ...
    - **目标配置**：
      ...
      - `custom` - 检测自定义URL
  - 测试要求：
    - [ ] 创建测试脚本 test/task9.sh，验证诊断工具的准确性和实用性。
    - [ ] 测试不同网络环境下的诊断效果（正常、代理、离线、限速）。
    - [ ] 验证错误分类的准确性和建议的有效性。
    - [ ] 测试诊断性能和超时处理。
    - [ ] 模拟各种网络问题场景进行验证。
  - 测试步骤：
