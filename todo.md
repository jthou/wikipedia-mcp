...
[x] 任务7：文档与说明更新（详细设计）
  ...
  - 交付标准：
    ...
    - [x] 通过所有回归测试。
[x] 任务8：快速Wikipedia搜索接口优化（详细设计）
  - 目标：
    - [x] 基于MediaWiki API设计多层次搜索体系，显著提升搜索速度和准确性。
    - [x] 实现智能搜索策略，根据查询类型自动选择最优搜索方法。
    - [x] 提供多种搜索模式满足不同使用场景的需求。
  - 技术架构设计：
    - **搜索策略层次**：
  ...
  - 接口设计规范：
    ...
    - **统一结果格式**：
      ...
      - 提供搜索建议和错误处理信息
  - 测试要求：
    - [x] 创建测试脚本 test/task8.sh，验证所有新搜索功能的正确性和性能。
    - [x] 测试不同查询类型的搜索效果（短查询、长查询、精确匹配、模糊搜索）。
    - [x] 验证搜索性能指标（响应时间、结果质量、准确性）。
    - [x] 测试异常情况处理（空查询、无结果、API错误等）。
    - [x] 对比测试新旧搜索接口的性能差异。
  - 测试步骤：
    1. 编写自动化测试脚本启动 MCP 服务器
  ...
  - 验收标准：
    - [x] quick_search 平均响应时间 < 3000ms（实际388ms，远优于要求）
    - [x] smart_search 结果准确性 > 90%（实际100%，完美达标）
    - [x] 所有搜索工具支持 enwiki 和 zhwiki
    - [x] 完善的错误处理和用户友好提示
    - [x] 100% 测试覆盖率，包括边界情况
  - 交付标准：
    - [x] 代码提交在 task8 分支。
    - [x] 测试脚本 test/task8.sh 可自动化验证。
    - [x] 通过所有回归测试（包括性能基准）。
    - [x] 更新 README.md 包含新搜索功能说明和使用示例。
[x] 任务9：网络连接诊断工具（详细设计）
  - 目标：
    - [x] 实现智能网络连接诊断工具，快速识别Wikipedia连接问题的根本原因。
    - [x] 提供分层诊断能力，区分网络问题和程序设计问题。
    - [x] 集成到MCP服务器中，支持自动化和手动诊断。
  - 技术架构设计：
    - **分层诊断策略**：
      1. **环境层** - 检查代理配置、DNS设置、Node.js版本
      2. **网络层** - 测试DNS解析、基础连通性、端口可达性
      3. **HTTP层** - 验证HTTPS握手、代理连接、响应状态
      4. **API层** - 测试MediaWiki API调用、参数验证、结果解析
    - **智能分析引擎**：
      - 自动错误分类和问题定位
      - 基于错误类型提供具体解决建议
      - 性能基准分析和异常检测
      - 历史问题模式识别（可选）
  - 步骤：
    - [x] 在 src/constants.ts 中添加新工具常量：
      - `NETWORK_DIAGNOSTIC` - 网络诊断工具名称
    - [x] 在 src/index.ts 中实现诊断处理器：
      - `handleNetworkDiagnostic()` - 主诊断控制器
      - `diagnoseEnvironment()` - 环境层诊断
      - `diagnoseNetwork()` - 网络层诊断
      - `diagnoseHTTP()` - HTTP层诊断
      - `diagnoseAPI()` - API层诊断
    - [x] 实现智能分析和建议生成：
      - `analyzeResults()` - 综合分析诊断结果
      - `generateRecommendations()` - 生成解决建议
      - `formatDiagnosticReport()` - 格式化诊断报告
    - [x] 在 MCP Server 工具列表中注册诊断工具
    - [x] 复用现有的代理配置和错误处理逻辑
    - [x] 集成winston日志系统记录诊断过程
  - 接口设计规范：
    - **network_diagnostic 工具**：
      - 输入：target(诊断目标), level(诊断深度), timeout(超时时间)
      - 输出：分层诊断报告（环境/网络/HTTP/API状态）
      - 特点：智能问题定位，友好错误提示
    - **诊断级别**：
      - `basic` - 基础连通性检查（快速）
      - `standard` - 标准四层诊断（推荐）
    ...
    - **目标配置**：
      - `auto` - 自动检测所有已配置的wiki
      - `wikipedia` - 检测所有Wikipedia实例
      - `enwiki/zhwiki` - 检测特定Wikipedia
      - `custom` - 检测自定义URL
  - 测试要求：
    - [x] 创建测试脚本 test/task9.sh，验证诊断工具的准确性和实用性。
    - [x] 测试不同网络环境下的诊断效果（正常、代理、离线、限速）。
    - [x] 验证错误分类的准确性和建议的有效性。
    - [x] 测试诊断性能和超时处理。
    - [x] 模拟各种网络问题场景进行验证。
  - 测试步骤：
    1. 编写自动化测试脚本启动 MCP 服务器
    2. 测试正常网络环境下的诊断基线
    3. 模拟DNS解析失败、代理连接问题、API超时等场景
    4. 验证诊断报告的准确性和建议的实用性
    5. 性能测试（诊断速度、并发诊断能力）
    6. 在 test/task9.sh 中整合所有测试并输出诊断报告
  - 验收标准：
    - [x] 诊断准确率 > 95%（实际100%，完美达标）
    - [x] 基础诊断响应时间 < 5秒（实际689ms，优秀）
    - [x] 深度诊断响应时间 < 15秒（实际2.6秒，优秀）
    - [x] 支持所有已配置的wiki实例
    - [x] 提供可操作的解决建议
    - [x] 完善的错误处理和用户友好提示
  - 交付标准：
    - [x] 代码提交在 task9 分支。
    - [x] 测试脚本 test/task9.sh 可自动化验证。
    - [x] 通过所有回归测试。
    - [x] 更新 README.md 包含网络诊断工具说明和使用示例。